...
const getUnavailableSpace = ({
  ...
  const info = (type) => {
    ...
    // switch over types and return thickness and minDistance / 

    return Config.furnishing[match];
  };

  console.log(original_assets)
  ...
  // sort descending by position y so we get all items from bottom to top
  Object.keys(sections).forEach(key => sections[key].sort((a, b) => a.bottom - b.bottom));

  Object.keys(sections).forEach(sectionKey => { // creat unavailable spaces, used for dragging
    ...
    if (shelfTypes.includes(type)) {
      unavailableSpaces.push({ // top
        ...
        type: 'top',
        availableTop: section[0].availableTop,
        availableBottom: section[0].availableBottom,
      })
    ...
    if (section.length > 1) {
      section.forEach((space, i) => {
        ...
        if (i === (section.length - 1) && space.topAsset !== 'none') {
          const asset = info(space.topAsset);
         
          unavailableSpaces.push({
            top: elementsTop,
            bottom: elementsTop - (asset.minDistance * 2) - asset.thickness1 ,
            width: section[0].width,
            height: asset.thickness1 + (asset.minDistance * 2),
            posX: space.posX,
            posY: elementsTop - (((asset.minDistance * 2) + asset.thickness1) / 2),
            posZ: depth + 0.5,
            inDivider: false,
            xIndex: parseInt(sectionKey),
            type: space.topAsset,
        ...
        } else if (i === 0 && space.bottomAsset !== 'none') {
          return console.log('Bottom no space', space);
        } else {

          if (!section[i + 1]) return;
          const bottom = space.top;
          const top = section[i + 1].bottom;

          const item = furnishingAssets.find(f => f.xIndex === parseInt(sectionKey) && f.position[1] >= bottom && f.position[1] <= top);

          const height = top - bottom - item.scale[1]; // shelf height is: 2 * 14 + 1.9 = 30 rounded

          // if item is the one dragged, make available
          ...
          unavailableSpaces.push({
            ...
            height: height,
            posX: space.posX,
            posY: bottom + (height / 2) + item.scale[1],
            posZ: depth + 0.5,
            inDivider: false,
            xIndex: parseInt(sectionKey),
            type: item.type,
...
//   original_assets.forEach(asset => { // creat unavailable spaces, used for dragging
//     const {xIndex, position, scale} = asset;
//     const assetInfo = info(asset.type);

//     const obj = { // top
//       top: position[1] + (scale[1] / 2) + assetInfo.minDistance,
//       bottom: position[1] - (scale[1] / 2) - assetInfo.minDistance,
//       width: sections[xIndex][0].width,
//       height: (2 * assetInfo.minDistance) + (scale[1]),
//       posX: position[0],
//       posY: position[1],
//       posZ: depth + 0.5,
//       inDivider: false,
...
export const unavailableSpace = ({
  ...
  const dividerArrays = {};
  const updatedAssets = [...furnishingAssets];
