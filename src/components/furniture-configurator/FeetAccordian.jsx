...
import { ErrorMessage, Field, Form, Formik } from "formik"; // , useFormik
...
import FeetImage from "/images/furniture_configurator/feetPositionImage.png?url";
...
export const HandlesAccordianAfter = ({
  ...
  const addVariationSubmit = async (values) => {
    ...
    const { data } = await AddDrawerList(id, formData, "addList");
    if (data) {
  ...
  const variationUpdateSubmit = async (values) => {
    ...
    const { data } = await UpdateList(id, formdata, "updateList");
    if (data) {
      setVariationList(data?.data);
      toast.success(data?.message);
    }
  ...
  const DeleteSelectedVariation = async (id, index) => {
    const { data } = await DeleteVariation(id, index, "deletevariation");
    if (data) {
      setVariationList(data?.data);
      toast.success(data?.message);
    }
