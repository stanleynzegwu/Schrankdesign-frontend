...
const MainMaterial = () => {
    ...
    useEffect(() => {
        const GetVeriable = async () => {
            const { data, error } = await GetallcalculationVeriables("getVariables")
            if (data) {
                setIsLoading(false)
            ...
            <SubLayout tabs={tabs}>
                ...
                <div className="flex gap-[20px] flex-col xl:flex-row">
                    <VeriableNames
                        ...
                        isLoading={isLoading}
                    />
                    {AddNewVeriableFlag && <NewVeriable setVeriableList={setVeriableList} />}
                    {OpenVeriableFunction && <VariableFunctions
                        VeriableList={VeriableList}
