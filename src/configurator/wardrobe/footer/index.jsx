...
const Footer = React.memo(function Footer() {
  ...
  // Initialzie drawer_asset_info object for drawer asset (Drawer-Runner)
  let drawer_asset_info = {};
  ...
  useEffect(() => {
    ...
    for (const asset of doorAssets) {
      ...
      const height = getDoorHeight(asset.scale[1], asset.topAsset, asset.bottomAsset);
    ...
    //platesInfo comes from zustandFurnishing store, furnishingPlates now is the one we make of the furnishingAssets
    ...
    for (const plate of totalPlates) {
      ...
      plate_info[key]["Cut-Plate-m-pcs"] += ((plate.length + plate.depth) * 2) / 100;
      plate_info[key]["Plate-m2-type"] +=
        (((plate.length * plate.depth) / 10000) * (100 + calcInfo.info["Plate-waste"])) / 100;
      ...
      if (calcInfo["plates_types"][plate.id]) {
        ...
        if (
          ...
          if (edge_type.includes("V")) plate_info[key]["Edge-m-type"] += plate.length / 100;
          if (edge_type.includes("H")) plate_info[key]["Edge-m-type"] += plate.length / 100;
          if (edge_type.includes("L")) plate_info[key]["Edge-m-type"] += plate.depth / 100;
          if (edge_type.includes("R")) plate_info[key]["Edge-m-type"] += plate.depth / 100;
        }
        ...
        for (const asset_data of calcInfo["plates_types"][plate.id].list) {
          ...
          if (asset_data.functions[0] === "Add-Distance") {
            ...
            asset_info[asset_data.child_config_id[0]] += Number(asset_data.qty[0]);
          }
        }
      }
    }
    ...
    for (const plate of totalPlates) {
      ...
      if (calcInfo["plates_types"][plate.id]) {
        ...
        if (edgeType.includes("V"))
          result["Edge-Length"] = roundNumber(result["Edge-Length"] + plate.length * 10);
        if (edgeType.includes("H"))
          result["Edge-Length"] = roundNumber(result["Edge-Length"] + plate.length * 10);
        if (edgeType.includes("L"))
          result["Edge-Length"] = roundNumber(result["Edge-Length"] + plate.depth * 10);
    ...
    //drawer_asset_info => Drawer-runner
    partlist_data.assets = { ...drawer_asset_info, ...asset_info };

    //console.log(partlist_data);
    ...
    Object.keys(plate_info).forEach((key) => {
      plate_cost[key]["Cut-Plate-m-pcs"] = roundNumber(plate_info[key]["Cut-Plate-m-pcs"]);
      plate_cost["Total"]["Cut-Plate-m-pcs"] = roundNumber(
        plate_cost["Total"]["Cut-Plate-m-pcs"] + plate_cost[key]["Cut-Plate-m-pcs"]
      );

      plate_cost[key]["Plate-m2-type"] = roundNumber(plate_info[key]["Plate-m2-type"]);
      ...
      plate_cost["Total"]["Plate-pcs-type"] = roundNumber(
        plate_cost["Total"]["Plate-pcs-type"] + plate_cost[key]["Plate-pcs-type"]
      );

      plate_cost[key]["Cut-Plate-time-pcs-type"] = roundNumber(
        (plate_info[key]["Plate-pcs-type"] * calcInfo.info["Cut-Plate-time-pcs"]) / 60
      );
      plate_cost["Total"]["Cut-Plate-time-pcs-type"] = roundNumber(
        plate_cost["Total"]["Cut-Plate-time-pcs-type"] + plate_cost[key]["Cut-Plate-time-pcs-type"]
      );

      plate_cost[key]["Cut-Plate-time-m-type"] = roundNumber(
        (plate_cost[key]["Cut-Plate-m-pcs"] * calcInfo.info["Cut-Plate-time-m"]) / 60
      );
      plate_cost["Total"]["Cut-Plate-time-m-type"] = roundNumber(
        plate_cost["Total"]["Cut-Plate-time-m-type"] + plate_cost[key]["Cut-Plate-time-m-type"]
      );
      ...
      plate_cost["Total"]["Cut-Plate-time-type"] = roundNumber(
        plate_cost["Total"]["Cut-Plate-time-type"] + plate_cost[key]["Cut-Plate-time-type"]
      );
      ...
      plate_cost[key]["Plate-mat-cost-type"] = roundNumber(
        plate_cost[key]["Plate-m2-type"] * calcInfo.plates_edges[key]?.plate_price || 0
      );
      plate_cost["Total"]["Plate-mat-cost-type"] = roundNumber(
        plate_cost["Total"]["Plate-mat-cost-type"] + plate_cost[key]["Plate-mat-cost-type"]
      );

      plate_cost[key]["Plate-cost-type"] = roundNumber(
        plate_cost[key]["Cut-Plate-time-cost-type"] + plate_cost[key]["Plate-mat-cost-type"]
      );
      plate_cost["Total"]["Plate-cost-type"] = roundNumber(
        plate_cost["Total"]["Plate-cost-type"] + plate_cost[key]["Plate-cost-type"]
      );
    });
    ...
    Object.keys(plate_info).forEach((key) => {
      edge_cost[key]["Edge-m-type"] = roundNumber(plate_info[key]["Edge-m-type"]);
      ...
      edge_cost["Total"]["Edge-time-cost-type"] = roundNumber(
        edge_cost["Total"]["Edge-time-cost-type"] + edge_cost[key]["Edge-time-cost-type"]
      );

      edge_cost[key]["Edge-mat-cost-type"] = roundNumber(
        edge_cost[key]["Edge-m-type"] * calcInfo.plates_edges[key]?.edge_price || 0
      );
      edge_cost["Total"]["Edge-mat-cost-type"] = roundNumber(
        edge_cost["Total"]["Edge-mat-cost-type"] + edge_cost[key]["Edge-mat-cost-type"]
      );

      edge_cost[key]["Edge-cost-type"] = roundNumber(
        edge_cost[key]["Edge-time-cost-type"] + edge_cost[key]["Edge-mat-cost-type"]
      );
    ...
    /****** were we had this asset_info , I changed to { ...drawer_asset_info, ...asset_info } , so it includes drawer assets. I think i'll make it a variable ******/
    /****** It's basically an object with drawer assets(Drawer-runner-(dynamic number)) and other assets like(Fachboden-Träger)*/
    Object.keys({ ...drawer_asset_info, ...asset_info }).forEach((key) => {
    ...
    Object.keys({ ...drawer_asset_info, ...asset_info }).forEach((key) => {
      ...
      asset_cost[name]["Asset-pcs-type"] = { ...drawer_asset_info, ...asset_info }[key];
      //asset_cost[name]["Asset-pcs-type"] = asset_info[key];
      asset_cost["Total"]["Asset-pcs-type"] = roundNumber(
        asset_cost["Total"]["Asset-pcs-type"] + asset_cost[name]["Asset-pcs-type"]
      );
      ...
      asset_cost["Total"]["Asset-cost-type"] = roundNumber(
        asset_cost["Total"]["Asset-cost-type"] + asset_cost[name]["Asset-cost-type"]
      );
    });
    ...
    Object.keys({ ...drawer_asset_info, ...asset_info }).forEach((key) => {
      ...
      con_cost["Con-Asset-time-total"] = roundNumber(
        con_cost["Con-Asset-time-total"] +
          { ...drawer_asset_info, ...asset_info }[key] * calcInfo.assets[key].con_asset_time_pcs
        //con_cost["Con-Asset-time-total"] + asset_info[key] * calcInfo.assets[key].con_asset_time_pcs
      );
    });
    ...
    Object.keys({ ...drawer_asset_info, ...asset_info }).forEach((key) => {
      ...
      packing_cost["Pack-Asset-time-total"] = roundNumber(
        (packing_cost["Pack-Asset-time-total"] +
          { ...drawer_asset_info, ...asset_info }[key] * calcInfo.assets[key].pack_asset_time_pcs) /
          //asset_info[key] * calcInfo.assets[key].pack_asset_time_pcs) /
          60
      );
    });
    packing_cost["Pack-Plate-time-total"] = roundNumber(
      (packing_cost["Plates-pcs-total"] * calcInfo.info["Pack-Plate-time-pcs"]) / 60
    );
    ...
    priceInfo["Sum-till-Fee2"] = roundNumber(priceInfo["Sum-till-Fee1"] + calcInfo.info["Fee-2"]);
      ...
      <div className="w-full h-[63px] flex-none bg-[#E5E5E5] z-50">
        <div className="flex justify-between items-center h-full px-[12px] ">
          <div className="flex justify-start items-center gap-2">
            <span className="text-[#000] text-[28px]">{priceData["End-Price"]}€</span>
            <div className="flex flex-col justify-start">
              <span className="text-[#000] text-[12px]">inkl. MwSt zzgl. Lieferkosten</span>
              <span className="text-[#36695C] text-[15px]">Lieferung in 4-5 Wochen </span>
            </div>
          </div>
              ...
              <>
                <Button
                  ...
                  onClick={() => {
                    ...
                    doc.text("PartList Information", doc.internal.pageSize.width / 2, 15, {
                      align: "center",
                    });
                    ...
                    doc.autoTable({
                      startY: 35,
                      head: [
                        Object.keys(partlistData.plates[Object.keys(partlistData.plates)[0]])
                      ...
                      body: Object.keys(partlistData.plates).map((item) => {
                        ...
                        Object.keys(partlistData.plates[item])
                          .filter((key) => key !== "Index")
                          .forEach((col) => {
                            if (col === "Length" || col === "Depth" || col === "Edge-Length")
                    ...
                    doc.autoTable({
                      ...
                      body: Object.keys(priceData["Edge-Cost"])
                        ...
                        .map((item) => {
                          return {
                            "Edge-mat-name": item,
                            "Edge-m-type": `${priceData["Edge-Cost"][item]["Edge-m-type"]} ${
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    doc.text("Calculation Information", doc.internal.pageSize.width / 2, 15, {
                      align: "center",
                    });
                    ...
                    doc.autoTable({
                      ...
                      head: [
                        {
                          ...
                          ...Object.keys(
                            priceData["Plate-Cost"][Object.keys(priceData["Plate-Cost"])[0]]
                          ).reduce((acc, key) => {
                            acc[key] = key;
                            return acc;
                          }, {}),
                      ...
                      body: Object.keys(priceData["Plate-Cost"])
                        ...
                        .map((item) => {
                          ...
                          Object.keys(priceData["Plate-Cost"][item]).forEach((col) => {
                    ...
                    doc.autoTable({
                      ...
                      head: [
                        {
                          ...
                          ...Object.keys(
                            priceData["Edge-Cost"][Object.keys(priceData["Edge-Cost"])[0]]
                          ).reduce((acc, key) => {
                            acc[key] = key;
                            return acc;
                          }, {}),
                      ...
                      body: Object.keys(priceData["Edge-Cost"])
                        ...
                        .map((item) => {
                          ...
                          Object.keys(priceData["Edge-Cost"][item]).forEach((col) => {
                    ...
                    doc.autoTable({
                      ...
                      head: [
                        {
                          ...
                          ...Object.keys(
                            priceData["Asset-Cost"][Object.keys(priceData["Asset-Cost"])[0]]
                          ).reduce((acc, key) => {
                            acc[key] = key;
                            return acc;
                          }, {}),
                      ...
                      body: Object.keys(priceData["Asset-Cost"])
                        ...
                        .map((item) => {
                          ...
                          Object.keys(priceData["Asset-Cost"][item]).forEach((col) => {
