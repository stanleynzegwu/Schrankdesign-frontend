...
const ExteriorPlates = React.memo(function ExteriorPlates() {
  ...
  const plinthXInfo = useMemo(() => calculatePlinthXInfo(elementsWidths), [elementsWidths]);
  ...
  const glidersPosXInfo = useMemo(() => calculateGlidersPosXInfo(elementsWidths), [elementsWidths]);
    ...
    <>
            ...
            <Plate
              args={[
                ...
                korpusType == Config.korpusType.innerShap2
                  ...
                  : info.size[1],
                korpusType == Config.korpusType.innerShap2 ? info.size[2] + 1.9 : info.size[2],
              ]}
              position={[
                info.pos[0],
                korpusType == Config.korpusType.outerShap ||
                ...
                korpusType == Config.korpusType.innerShap2
                  ...
                  : info.pos[1],
                korpusType == Config.korpusType.innerShap2 ? info.pos[2] + 0.95 : info.pos[2],
              ]}
              ...
              <Geometry useGroups>
                <Base>
                  ...
                  korpusType == Config.korpusType.outerShap ? (
                    ...
                    <roundedBoxGeometry
                      args={[
                        withFeet ? info.size[1] - Config.plate.plinthHeight : info.size[1],
                        info.size[0],
                        info.size[2],
                        4,
                        Config.plate.radius,
                ...
                >
                  <boxGeometry
                    args={
                      enableCutout ? [cutoutHeight, Config.plate.thickness, cutoutDepth] : [0, 0, 0]
                    }
                  />
                </Subtraction>
              </Geometry>
      ...
      {plinthXInfo.map((info, index) => (
        <group key={index}>
          {withOutFeet === false && withFeet === false && hanging === false && (
            <Plate
              args={[info.width, Config.plate.plinthHeight, Config.plate.thickness]}
              position={[
                info.xPos,
          ...
          korpusType === Config.korpusType.innerShap2 ? (
            <Plate
              ...
              position={[
                info.xPos,
                withOutFeet === true || hanging === true
                  ? (baseType == Config.baseType.panel ? 0 : Config.glider.height) +
      ...
      {glidersPosXInfo.map((info, index) => (
        <group key={index} visible={baseType === Config.baseType.gliders}>
          <Cylinder
            args={[Config.glider.radius, Config.glider.radius, Config.glider.height]}
            position={[
              info,
