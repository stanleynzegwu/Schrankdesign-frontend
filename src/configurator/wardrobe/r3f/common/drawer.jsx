...
const Drawer = React.memo(function Drawer(props) {
  ...
  const positionY = (
    ...
    if ( (korpusType === Config.korpusType.empty || korpusType === Config.korpusType.outerShap) && topVisible === false && topAsset === "none") {
    ...
    ) && bottomVisible === false  && bottomAsset === "none") {
      return positionY -= 0.475
    }
    if ( (korpusType === Config.korpusType.topShap ||
      korpusType === Config.korpusType.uShap ||
      korpusType === Config.korpusType.innerShap ||
      korpusType === Config.korpusType.innerShap2) && topVisible === false
    ) {
      if ( korpusType === Config.korpusType.innerShap2) {
        return positionY
      }
    ...
    if ( (korpusType === Config.korpusType.innerShap ||
      korpusType === Config.korpusType.innerShap2) && bottomVisible === false
    ) {
      if ( korpusType === Config.korpusType.innerShap2) {
        return positionY
      }
  ...
  const shelfPlate = () => {
    ...
    for (let i = 0; i < drawerGroupScale.length - 1; i++) {
      if (i === 0) {
        ...
        positionY +=  drawerGroupScale[i] + Config.furnishing.shelf.thickness1 + topShelfDistance
          + Config.furnishing.drawer.bottomShelfDistance
      }
    ...
    <group visible={visible}>
      ...
      {drawerGroup > 1 ? (
        ...
        <Plate
          ...
          position={[
            positionX(korpusType),
            positionY(topShelfDistance, drawerConfig, topVisible, bottomVisible, topAsset, bottomAsset, korpusType),
